...
function getUiPathFromConfig() {
    ...
    try {
        ...
        console.log(
            `Resolve alias "${uiAlias}" -> "${path.relative(process.cwd(), uiPath)}"`,
        );
...
async function restructureComponent(componentName, uiPath, inputComponent) {
    try {
        let componentFile =
            inputComponent || path.join(uiPath, `${componentName}.tsx`);
        ...
        console.log(`${componentName}: restructured`);
...
async function main() {
    ...
    if (command === "init") {
        ...
        console.log("Created shadcnf.json configuration file");
        process.exit(0);
    }
    ...
    try {
        ...
        const output = execSync(`npx shadcn@latest add ${component}`, {
            encoding: "utf-8",
        });
...
function getComponentPathFromStdio(output) {
    console.log(output, "output from shadcn");
    ...
    if (match) {
        ...
        throw new Error("error parsing shadcn stdio");
    }
}
